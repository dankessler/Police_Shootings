...
#+BEGIN_SRC stan
  data {
    ...
    int<lower=0> C[nc][2]; // number of shootings for county i at time j
    int<lower=0> pDemo; // number of demographic predictors
  }
  transformed data {
    int<lower=0> p;
    p = 1 + 1 + pDemo + 1; // number of predictors (offset + race demographics + couny:time)
    vector[p] X[nc]; // demographic predictor for each county
    for (1 in nc){
      X[1][1]
        }
  }
